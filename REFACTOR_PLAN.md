...
5. **Notes** — `TextInput`, paragraph, `required=False`, `max_length=200`.
...
Required fields render Discord's native red asterisk and cannot be submitted empty; required selects therefore always yield exactly one value in `on_submit` (read via `self.field.component.value` / `.values` — with a defensive `IndexError` guard). The six **optional lobby settings** never enter the modal: they arrive as slash-command options (§4.2a below) already client-validated, threaded in via the modal's `optional_settings` dict (display name → value-or-None), and merged into the embed's settings fields after the mandatory three.
...
15a. **Derived moddedness (Revision 7):** `/modlfg` opens a modal titled "Post a Modded LFG" and its post shows `Modded Lobby: Yes`; `/lfg` shows `Modded Lobby: No`; picking a Weapon Randomizer in the modal renders it on the embed between Mid-Match Joining and Enemy Outlines; `/lfg` then `/modlfg` within 60s → second refused (shared cooldown store).
