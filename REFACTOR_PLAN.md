...
1. Member types `/lfg` anywhere it's usable (recommendation: everywhere — see §4.2 on channel restriction), optionally fills any of the six **optional lobby settings** right in the command picker (`first_to` 1-50, `lobby_type`, `friendly_fire`, `mid_match_joining`, `weapon_randomizer`, `enemy_outlines` — all dropdown/range options Discord validates client-side, §4.2a), and hits Enter.
...
4. Otherwise `interaction.response.send_modal(LFGPostModal(...))` — a valid **initial** response to an application-command interaction (Discord interaction callback type 9, MODAL, is available for APPLICATION_COMMAND interactions — verified fact). Modal **"Post an LFG"** — exactly at the 5-component cap (§4.3): `Lobby ID` (short text, required), `Max Players` (required select: 2/3/4), `Gamemode` (required select: FFA/Teams), `Modded Lobby` (required select: Yes/No), `Notes` (paragraph, optional, `max_length=200`). Discord blocks submission until the required fields are filled.
...
4. **Modded Lobby** — string select, required, options `Yes` / `No`.
...
2. Owner runs `[p]slash enable lfg`, `[p]slash enable lfgpings`, `[p]slash enable testlfg` (or `[p]slash enablecog LFG`), then `[p]slash sync`. `[p]slash list` to verify; members may need Ctrl+R.
...
1. **Enable/sync:** `[p]slash enable lfg`, `[p]slash enable lfgpings`, `[p]slash enable testlfg`, `[p]slash sync` → `/lfg` and `/lfgpings` appear in the command picker for everyone, `/testlfg` for Administrators only (Ctrl+R if not); `[p]slash list` shows all three enabled with no pending markers.
...
14. **Mandatory modal fields (Revision 6):** the modal cannot be submitted with any of Max Players / Gamemode / Modded Lobby unselected (Discord blocks submission and marks them with a red asterisk); a submitted post's embed always carries Max Players, Gamemode, and Modded Lobby fields (identical names on modal and embed).
15. **Optional slash options (Revision 6):** `/lfg first_to:10 lobby_type:Private friendly_fire:Disabled` → those three fields appear on the embed with exactly those values; skipped options don't render. `first_to` rejects 0 and 51 client-side (Range 1-50). Same options work on `/testlfg`.
