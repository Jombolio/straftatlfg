...
**Target runtime:** Red 3.5 stable — 3.5.24 at time of writing (verified to bundle discord.py 2.7.1). Every API feature used in core scope (app commands, `send_modal`, `Modal`/`TextInput`, `guild_only`, ephemeral defer/followup) is available from discord.py 2.0 / Red 3.5.0 (2023-05-04, when `[p]slash` shipped per the 3.5.0 changelog). `min_bot_version` is nevertheless pinned at **3.5.10** (dpy 2.4.0) as a deliberate conservative choice: the planned follow-ups that genuinely need dpy 2.4 (`DynamicItem`, §9.5; the `allowed_contexts` spelling, §4.2) then require no later `info.json` bump (§5 PR 1).
...
1. Member types `/lfg` anywhere it's usable (recommendation: everywhere — see §4.2 on channel restriction) and hits Enter.
...
4. Otherwise `interaction.response.send_modal(LFGPostModal(...))` — a valid **initial** response to an application-command interaction (Discord interaction callback type 9, MODAL, is available for APPLICATION_COMMAND interactions — verified fact). Modal **"Post an LFG"**: `Lobby ID` (short, required, `max_length=10`, placeholder `Numbers only, e.g. 12345`) and `Notes` (paragraph, optional, `max_length=200`) — well under the 5-component modal cap.
...
- **Callback error handling:** a `cog_app_command_error` handler covers unexpected exceptions in the `/lfg` callback itself (gate/peek/`send_modal` path) and in `/lfgpings` — it logs to `red.straftatlfg` and sends a generic ephemeral apology, via `interaction.response.send_message` if the response is still unsent, else `interaction.followup.send`. Without it, a callback exception surfaces to the member as Discord's bare "The application did not respond" with nothing logged, breaking the ephemeral-feedback goal for the failure case. (Modal submission errors are handled separately by `LFGPostModal.on_error`, §4.3.)
...
- `async def refund_cooldown(member, stamp)` — **compare-and-clear**: clears cache + Config only if the stored stamp equals `stamp`, so a stale failure can never erase a newer legitimate stamp. Callers may only invoke it while their per-invocation committed-stamp marker is still set — i.e. before the #new-lfg send has succeeded (see ordering below).
...
- `cog_app_command_error` handler (§4.2): logs to `red.straftatlfg`, generic ephemeral apology via response-if-unsent else followup.
...
- `info.json`: `min_bot_version: "3.5.10"` — a deliberately conservative pin: everything PR 1 actually uses is available from Red 3.5.0 / dpy 2.0, but pinning 3.5.10 (dpy 2.4.0) means the planned follow-ups that do need dpy 2.4 (`DynamicItem`, §9.5; the `allowed_contexts` spelling, §4.2) require no later bump — and `end_user_data_statement: "This cog stores the timestamp of each user's last LFG post to enforce a posting cooldown."` — shipped in the **same PR** that adds the member timestamp store.
...
1. **Enable/sync:** `[p]slash enable lfg`, `[p]slash enable lfgpings`, `[p]slash sync` → `/lfg` and `/lfgpings` appear in the command picker (Ctrl+R if not); `[p]slash list` shows both enabled with no pending markers.
...
13. *(If PR 2 ships)* **Atomic swap self-heal:** two region roles by hand, one reaction → exactly one region role afterward; a managed booster role survives the swap.
