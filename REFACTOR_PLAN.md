...
15. **Optional slash options (Revisions 6-7):** `/lfg first_to:10 lobby_type:Private friendly_fire:Disabled` → those three fields appear on the embed with exactly those values; skipped options don't render. `first_to` rejects 0 and 51 client-side (Range 1-50). Same options work on `/lfgmod`, `/testlfg`, and `/testlfgmod`.
15a. **Derived moddedness (Revision 7):** `/lfgmod` opens a modal titled "Post a Modded LFG" whose Notes field asks for the mods being run (the `/lfg` modal keeps the generic Notes copy); its post shows `Modded Lobby: Yes`; `/lfg` shows `Modded Lobby: No`; picking a Weapon Randomizer in the modal renders it on the embed between Mid-Match Joining and Enemy Outlines; `/lfg` then `/lfgmod` within 60s → second refused (shared cooldown store).
